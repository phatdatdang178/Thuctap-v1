...
namespace FestivalHoa.Properties.Services.Core
{
    public class MenuCongDanService : BaseService, IMenuCongDanService
    {
        ...
        {
            _context = context;
            BaseMongoDb = new BaseMongoDb<Menu, string>(_context.MENUCONGDAN);
        }
        ...
        {
            var listDonVi = await _context.MENUCONGDAN.Find(_ => _.IsDeleted != true).SortBy(donVi => donVi.Level).ToListAsync();
        ...
        {
            if (model == null || _context.MENUCONGDAN == null)
                throw new ResponseMessageException().WithException(DefaultCode.DATA_NOT_FOUND);

            var entity = await _context.MENUCONGDAN.Find(x => !x.IsDeleted && x.Id == model.Id).FirstOrDefaultAsync();
            ...
            var result = await _context.MENUCONGDAN.UpdateOneAsync(filter, update);
        ...
        public async Task<dynamic> DeleteAc(MenuListShort model)
        {
            try
            {
                ...
                var filter = Builders<Menu>.Filter.Where(x => x.Id == model.Id);
                var entity = await _context.MENUCONGDAN
                ...
                if (entity.ListAction != null)
                {
                    ...
                    {
                        var updatedEntity = await _context.MENUCONGDAN
                            .Aggregate()
                            .Match(filter)
                            .SortByDescending(x => x.Sort)
                            .Project<MenuListShort1>(MenuModuel.Projection_BasicCommon)
